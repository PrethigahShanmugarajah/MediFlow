      ...
      <div className="homedocs-max-w mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {error ? (
          <div className="text-center mb-6">
            ...
            <button
              onClick={() => fetchDoctorsApi(setDoctors, setError, setLoading)}
        ...
        {loading ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 sm:gap-8">
            {Array.from({ length: previewCount }).map((_, i) => (
              <div
                key={i}
                className="animate-pulse bg-white rounded-3xl homedocs-shadow-md p-4 h-72"
              >
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 sm:gap-8">
            {preview.map((doctor) => (
              <article
                ...
                className="group relative bg-white rounded-3xl homedocs-shadow-md hover:homedocs-shadow-2xl transition transform duration-300 overflow-hidden"
              >
                {doctor.available ? (
                  <Link
